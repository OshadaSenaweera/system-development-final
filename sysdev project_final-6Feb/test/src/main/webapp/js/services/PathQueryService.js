...
sysdevinterface.service('pathQueryService', [ '$http', 'modifyMap', function ($http, modifyMap) {
  let that = this
  const baseUrl = 'http://localhost:9292/test/service/Direction'
	  //const baseUrl = 'https://maps.googleapis.com/maps/api/directions/json?key=AIzaSyCfAQLVnai8jlMNpyzOqg0Sjn-0gnSGa4I&origin="+markers[0].lat+","+markers[0].lng+"&destination="+markers[1].lat+","+markers[1].lng;'
  const key = "AIzaSyCfAQLVnai8jlMNpyzOqg0Sjn-0gnSGa4I";
    ...
    model['usedAlgorithm'] = model.selected.algorithm.internal_name
      switch(model['usedAlgorithm']){
        ...
        case 'Google Path (direct)':
          url = "https://maps.googleapis.com/maps/api/directions/json?key=AIzaSyCfAQLVnai8jlMNpyzOqg0Sjn-0gnSGa4I&origin="
         ...
         // alert('lat  '+markers[0].lat)
         // alert('lng  '+markers[0].lng)
          break;
        ...
        case 'Google Path (via Server [OBJ])':
         ...
         $http.get(url+"/obj/", requestObject)
         .then(response => {
        ...
        case 'Dijkstra Shortest Path':
         ...
         /*
            $http.get(baseUrl, requestObject)
           .then(response => {
                 that.routeResponse(model, response)
               })*/
                       
        	 url = "https://maps.googleapis.com/maps/api/directions/json?key=AIzaSyCfAQLVnai8jlMNpyzOqg0Sjn-0gnSGa4I&origin="
                +markers[1].lat+","+markers[1].lng+"&destination="+markers[0].lat+","+markers[0].lng;
        	 
         $http.get(url).success(function(response) {
        response = that.extractGeoJson(response);
        that.routeResponse(model, response);
          })
          System.out.println(markers[1].lng);
        	//alert('lat  '+markers[0].lat)
          //alert('lng  '+markers[0].lng)
              break;
